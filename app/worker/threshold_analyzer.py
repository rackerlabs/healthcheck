...
class ThresholdAnalyzer(BaseAnalyzer):
    ...
    def analyze_results(self, criteria, results):
        # return True if it passes threshold, false if not
        passes = 0
        fails = 0
    ...
    def process_canary(self, canary_id, project_id):
        ...
        canary_results = self.api_client.get_results(project_id, canary_id, sample_size)
        result_list = canary_results.get('results')
        change_health = self.analyze_results(criteria=canary_criteria, results=result_list)
        if change_health:
            if current_health != "RED":
