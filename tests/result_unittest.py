...
class ResultTest(unittest.TestCase):
    ...
    def test_get_results(self):
        ...
        get_canary = self.client.get('api/projects/1/canary/1/results', content_type='application/json')
        self.assertEquals(get_canary.status_code, 200)
        self.assertEquals(len(json.loads(get_canary.data).get('results')), results,
    ...
    def test_edit_result(self):
        ...
        self.assertEquals(result.status_code, 201)
        self.assertEquals(json.loads(result.data).get('id'), 1)
        edit_result = self.client.put('api/projects/1/canary/1/results/1', data=json.dumps(
            {'status': 'fail'}), headers={'content-type': 'application/json'})
        ...
        self.assertEquals(json_data.get('status'), 'fail',
                          "EXPECTED {}, GOT {}".format('fail', json_data.get('status')))
        self.assertEquals(json_data.get('failure_details'), "",
                          "EXPECTED {}, GOT {}".format("", json_data.get('failure_details')))
    ...
    def test_delete_result(self):
        ...
        self.assertEquals(json.loads(wrong_delete.data).get('message'), "result not found")
